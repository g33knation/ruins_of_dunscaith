...
fn main() {
    let opcode: u16 = 0x0009; // OP_Packet
    let seq: u16 = 0x0000;    // Sequence 0
    let key: u32 = 0;         // CRC Key 0
    ...
    println!("CRC for Op=0009 Seq=0000 Key=0 is {:02x?} (LE Bytes: {:02x} {:02x})", crc16, crc16 as u8, (crc16 >> 8) as u8);
}
