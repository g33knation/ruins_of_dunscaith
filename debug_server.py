...
# --- MAGIC BYTES ---
# Port 5998 in Little Endian (0x6E, 0x17)
...
def start_server():
    ...
    while True:
        ...
        try:
            ...
            # Find Magic Word "EverQuest"
            magic_idx = data.find(b"EverQuest")
            if magic_idx != -1:
                print("[UDP] Magic Word found! Sending Response...")
                
                # Construct Response
                # Echo Header + Magic
                response = bytearray()
                # Find end of magic word (null terminator)
                ...
                response.extend(PORT_BYTES)          # Port
                response.extend(STATUS_BYTES)        # Status
                ...
                print(f"[UDP] Sent Invite: Port bytes={PORT_BYTES.hex()} Status={STATUS_BYTES.hex()}")
        ...
        try:
            ...
            # Send Handshake
            # Size = 14 (12 bytes body + 2 bytes opcode)
            # OpCode = 1 (SessionReady)
            # Body = 12 bytes of zeros
            
            packet = bytearray()
            packet.extend((14).to_bytes(2, 'big'))   # Size (BE)
            packet.extend((1).to_bytes(2, 'big'))    # OpCode (BE)
            packet.extend(b'\x00' * 12)              # Body
            
            conn.sendall(packet)
            print("[TCP] Sent OP_SessionReady (Size: 14)")
            ...
            if data:
                ...
                if len(data) > 4:
                    ...
                    if opcode == 2:
                        print(">>> SUCCESS! Client sent OP_Login! <<<")
                        print("The configuration in this script is the GOLDEN KEY.")
                        sys.exit(0)
